...
from rehydra.test_utils.test_utils import find_parent_dir_containing, run_python_script
...
def bump_version(cfg: Config, package: Package, rehydra_root: str) -> None:
    if package.version_type == VersionType.SETUP:
        ver_file = Path(rehydra_root) / package.path / "setup.py"
        bump_version_in_file(cfg, package.name, ver_file)
    elif package.version_type == VersionType.FILE:
        ver_file = Path(rehydra_root) / package.path / package.version_file
        bump_version_in_file(cfg, package.name, ver_file)
    else:
        raise ValueError()
...
def main(cfg: Config) -> None:
    rehydra_root = find_parent_dir_containing(target="ATTRIBUTION")
    ...
    if cfg.action == Action.check:
        ...
        for package in cfg.packages.values():
            pkg_path = os.path.normpath(os.path.join(rehydra_root, package.path))
        ...
        for package in cfg.packages.values():
            ...
            ret = get_package_info(pkg_path)
            if ret.local_version == ret.latest_version:
                bump_version(cfg, package, rehydra_root)
