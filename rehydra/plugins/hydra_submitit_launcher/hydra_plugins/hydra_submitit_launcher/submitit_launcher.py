...
from rehydra.core.utils import JobReturn, filter_overrides, run_job, setup_globals
...
class BaseSubmititLauncher(Launcher):
    ...
    def __call__(
        ...
        with open_dict(sweep_config.rehydra.job) as job:
            # Populate new job variables
            job.id = submitit.JobEnvironment().job_id  # type: ignore
            sweep_config.rehydra.job.num = job_num
    ...
    def launch(
        ...
        log.info(
            ...
            f"{self.config.rehydra.sweep.dir}"
        )
        sweep_dir = Path(str(self.config.rehydra.sweep.dir))
        sweep_dir.mkdir(parents=True, exist_ok=True)
        if "mode" in self.config.rehydra.sweep:
            mode = int(str(self.config.rehydra.sweep.mode), 8)
