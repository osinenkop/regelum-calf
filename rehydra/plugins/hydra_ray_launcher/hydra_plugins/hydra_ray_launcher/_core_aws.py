...
from rehydra.core.utils import JobReturn, configure_log, filter_overrides, setup_globals
...
def launch(
    ...
    with tempfile.TemporaryDirectory() as local_tmp_dir:
        ...
        for idx, overrides in enumerate(job_overrides):
            ...
            log.info(f"\t#{idx} : {ostr}")
            sweep_config = launcher.rehydra_context.config_loader.load_sweep_config(
        ...
        _pickle_jobs(
            ...
            rehydra_context=launcher.rehydra_context,
            sweep_configs=sweep_configs,  # type: ignore
            task_function=launcher.task_function,
            singleton_state=Singleton.get_state(),
        )
        return launch_jobs(
            launcher, local_tmp_dir, Path(launcher.config.rehydra.sweep.dir)
        )
