...
from rehydra.errors import InstantiationException
from rehydra.types import ConvertMode, TargetConf
...
def _call_target(
    ...
    new_kwargs = kwargs.copy()  ## RCOGNITA CODE HERE
    for key in kwargs:
...
def _resolve_target(
    ...
    if isinstance(target, str):
        try:
            ...
            msg = f"Error locating target '{target}', set env var REHYDRA_FULL_ERROR=1 to see chained exception."
            if full_key:
                msg += f"\nfull_key: {full_key}"
            raise InstantiationException(msg) from e
...
def instantiate_node(
    ...
    *args: Any,
    ...
    if path is not None:
        ...
        if path in srccode.main.objects_created:
            return srccode.main.objects_created[path]
    # Return None if config is None
        ...
        if _is_target(node):
            ...
            if "callbacks__IGNORE__" in kwargs:
                for callback in kwargs["callbacks__IGNORE__"]:
                    if "." not in callback:
                        try:
                            _target_ = _locate("srccode.callback." + callback).attach(
                                _target_
                            )
                        except NameError:
            ...
            if "animations__IGNORE__" in kwargs:
                ...
                if "." not in animation:
                    animation = "srccode.callback." + animation
            ...
            if path:
                srccode.main.objects_created[path] = res
            return res
        else:
