...
from rehydra import MissingConfigException, version
from rehydra._internal.config_repository import IConfigRepository
from rehydra.core.config_store import ConfigStore
from rehydra.core.default_element import (
    ConfigDefault,
    DefaultsTreeNode,
    GroupDefault,
    InputDefault,
...
from rehydra.core.override_parser.types import Override
from rehydra.errors import ConfigCompositionException

from .deprecation_warning import deprecation_warning

cs = ConfigStore.instance()

cs.store(name="_dummy_empty_config_", node={}, provider="rehydra")
...
def _validate_self(
    ...
    if not has_self and has_non_override or len(defaults) == 0:
        # This check is here to make the migration from Rehydra 1.0 to Rehydra 1.1 smoother and should be removed in 1.2
        ...
        if containing_node.primary and has_config_content and has_non_override:
            msg = (
                ...
                f"See https://rehydra.cc/docs/1.2/upgrades/1.0_to_1.1/default_composition_order for more information"
            )
...
def _update_overrides(
    ...
    for d in defaults_list:
        ...
        legacy_rehydra_override = False
        if isinstance(d, GroupDefault):
            assert d.group is not None
            if not version.base_at_least("1.2"):
                legacy_rehydra_override = not d.is_override() and d.group.startswith(
                    "rehydra/"
                )

        if seen_override and not (
            d.is_override() or d.is_external_append() or legacy_rehydra_override
        ):
        ...
        if isinstance(d, GroupDefault):
            if legacy_rehydra_override:
                d.override = True
                url = "https://rehydra.cc/docs/1.2/upgrades/1.0_to_1.1/defaults_list_override"
                msg = dedent(
                    f"""\
            ...
            if d.override:
                if not legacy_rehydra_override:
                    seen_override = True
...
def _create_root(config_name: Optional[str], with_rehydra: bool) -> DefaultsTreeNode:
    ...
    if with_rehydra:
        root = DefaultsTreeNode(
            node=VirtualRoot(),
            children=[ConfigDefault(path="rehydra/config"), primary],
        )
    else:
...
def _create_defaults_list(
    ...
    overrides: Overrides,
    prepend_rehydra: bool,
    skip_missing: bool,
) -> Tuple[List[ResultDefault], DefaultsTreeNode]:
    root = _create_root(config_name=config_name, with_rehydra=prepend_rehydra)
...
def create_defaults_list(
    ...
    defaults, tree = _create_defaults_list(
        ...
        config_name,
        overrides,
        prepend_rehydra=prepend_rehydra,
        skip_missing=skip_missing,
    )
