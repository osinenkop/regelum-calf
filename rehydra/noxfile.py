...
def install_rehydra(session: Session, cmd: List[str]) -> None:
    # needed for build
    session.install("read-version", silent=SILENT)
    # clean install rehydra
    session.chdir(BASE)
    ...
    if not SILENT:
        ...
        session.run("pipdeptree", "-p", "rehydra-core")
...
def install_selected_plugins(
    ...
    # Test that we can import Rehydra
    session.run("python", "-c", "from rehydra import main", silent=SILENT)
...
def list_plugins(directory: str) -> List[Plugin]:
    ...
    for dir_name in _plugin_directories:
        ...
        if "rehydra_plugins" in os.listdir(abspath):
            module = "rehydra_plugins." + dir_name
            source_dir = "rehydra_plugins"
        else:
            module = dir_name
            source_dir = dir_name
...
def lint(session: Session) -> None:
    ...
    install_rehydra(session, ["pip", "install", "-e"])
    ...
    mypy_check_subdirs = [
        "examples/advanced",
        "examples/configure_rehydra",
        "examples/patterns",
        "examples/instantiate",
        "examples/tutorials/basic/your_first_rehydra_app",
        "examples/tutorials/basic/running_your_rehydra_app",
...
def lint_plugin(session: Session, plugin: Plugin) -> None:
    ...
    install_rehydra(session, install_cmd)
...
def _get_standalone_apps_dirs() -> List[Union[str, Path]]:
    ...
    apps.append(f"{BASE}/examples/advanced/rehydra_app_example")
    return apps
...
def test_core(session: Session) -> None:
    _upgrade_basic(session)
    install_rehydra(session, INSTALL_COMMAND)
    session.install("pytest")

    if not SKIP_CORE_TESTS:
        run_pytest(session, "build_helpers", "tests", *session.posargs)
    else:
        session.log("Skipping Rehydra core tests")
...
def test_plugins_vs_core(session: Session) -> None:
    ...
    # Run Rehydra tests to verify installed plugins did not break anything
    if not SKIP_CORE_TESTS:
