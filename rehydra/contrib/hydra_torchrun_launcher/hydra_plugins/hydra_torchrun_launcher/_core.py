...
def setup(
    ...
    *,
    ...
    launcher.rehydra_context = rehydra_context
    launcher.task_function = task_function

    c = config.rehydra.launcher
...
def launch(
    ...
    for idx, overrides in enumerate(job_overrides):
        ...
        runs.append(ret)
        configure_log(
            launcher.config.rehydra.rehydra_logging, launcher.config.rehydra.verbose
        )
    return runs
...
def wrapped_task_function(task_function, singleton_state, task_cfg):
    ...
    env_set = RehydraConfig.instance().cfg.rehydra.job.env_set
