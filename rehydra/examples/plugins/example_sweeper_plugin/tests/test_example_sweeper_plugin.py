...
from rehydra.test_utils.test_utils import TSweepRunner

from rehydra_plugins.example_sweeper_plugin.example_sweeper import ExampleSweeper
...
def test_launched_jobs(rehydra_sweep_runner: TSweepRunner) -> None:
    sweep = rehydra_sweep_runner(
        ...
        task_function=None,
        overrides=["rehydra/sweeper=example", "rehydra/launcher=basic", "foo=1,2"],
    )
    with sweep:
