"""Base infrastructure of srccode."""

import abc

from srccode import callback as cb
    ...
    def __call__(self, method):
        def new_method(self2, *args, **kwargs):
            ...
            if self.callbacks is None:
                try:
                    callbacks = SrccodeBase._metadata["main"].callbacks
...
class SrccodeType(abc.ABCMeta):
    """Srccode type that all classes in srccode share.
...
class SrccodeBase(metaclass=SrccodeType):
    """Base class designed to act as an abstraction over all srccode objects."""

    @classproperty
    def _metadata(self):
        return SrccodeBase.__metadata

    @_metadata.setter
    def _metadata(self, metadata):
        if not hasattr(SrccodeBase, f"_{SrccodeBase.__name__}__metadata"):
            SrccodeBase.__metadata = metadata
        else:
            raise ValueError(
    ...
    def __init__(self):
        """Initialize an object from srccode."""
        # if hasattr(self.__class__, "_attached"):
        #    existing_callbacks = [type(callback) for callback in srccode.main.callbacks]
        ...
        #            srccode.main.callbacks = [callback_instance] + srccode.main.callbacks
