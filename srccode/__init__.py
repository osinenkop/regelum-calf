...
from srccode.__internal.base import SrccodeBase
...
if "SRCCODE_DATA_DIR" not in os.environ:
    os.environ["SRCCODE_DATA_DIR"] = os.path.abspath(sys.path[0]) + "/srccode_data"
else:
    os.environ["SRCCODE_DATA_DIR"] = os.path.abspath(os.environ["SRCCODE_DATA_DIR"])

if "MLFLOW_TRACKING_URI" not in os.environ:
    os.environ["MLFLOW_TRACKING_URI"] = (
        f"file://{os.environ['SRCCODE_DATA_DIR']}/mlruns"
    )
...
class ComplementedConfig:
    """A config object, generated by ``srccode``'s config scenario."""
...
class ComplementedConfigList(ComplementedConfig):
    """A config object, generated by ``srccode``'s config scenario that corresponds to a complemented version of `ListConfig` from omegaconf."""
...
class ComplementedConfigDict(ComplementedConfig):
    """A config object, generated by ``srccode``'s config scenario that corresponds to a complemented version of `DictConfig` from omegaconf."""
...
class SrccodeExitException(Exception):
...
class main:
    """The decorator used to invoke ``srccode``'s config scenario.

    Use it to decorate your ``main`` function like so:
    ::

         @srccode.main(config_path=..., config_name=...)
         def my_main(config):
             ...
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Create an instance of ``srccode.main``.
        ...
        if logger is None:
            logger = logging.getLogger("srccode")
            # logger.info("Try")
        ...
        self.parser = self.SrccodeArgumentParser(description="Run with config ...")
            ...
            def app(
                ...
                with omegaconf.flag_override(cfg, "allow_objects", True):
                    ...
                    with Metadata(self.__class__.metadata):
                        ...
                        except SrccodeExitException as e:
                            res = e
