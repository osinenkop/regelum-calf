...
class Model(srccode.SrccodeBase, ABC):
...
class ModelQuadLin(Model):
    ...
    def __init__(
        ...
        random_weights_upper_bound: float = None,
        random_weights_lower_bound: float = None,
    ):
        ...
        if self.add_random_init_noise:
            ...
            self.weights = np.random.uniform(
                (
                    rg.ones(self.dim_weights) * random_weights_lower_bound
                    if random_weights_lower_bound is not None
                    else self.weight_min
                ),
                (
                    rg.ones(self.dim_weights) * random_weights_upper_bound
                    if random_weights_upper_bound is not None
                    else self.weight_max
                ),
                self.dim_weights,
            )
