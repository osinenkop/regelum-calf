...
class Objective(srccode.SrccodeBase, ABC):
...
class PendulumRunningObjective(RunningObjective):

    def __init__(self):
        pass

    def __call__(self, observation, action, is_save_batch_format: bool = False):
        ...
        angle = rg.arccos(1 - observation[:, 0])
        angle_vel = observation[:, 1]
        torque = action

        cost = angle**2 + 0.1 * angle_vel**2 + 0.001 * torque**2
