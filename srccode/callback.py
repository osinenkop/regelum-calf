...
import sys
import filelock
import srccode.__internal.base

import numpy as np
from typing import Dict, Any, Type

from srccode.event import Event
import torch
    ...
    class PassdownCallback(CallbackClass):
        def __call_passdown(self, obj, method, output):
            if (
                ...
                except srccode.SrccodeExitException as e:
                    raise e
...
class Callback(srccode.__internal.base.SrccodeBase, ABC):
...
class OnEpisodeDoneCallback(Callback):
    ...
    def is_target_event(self, obj, method, output):
        return isinstance(obj, srccode.scenario.Scenario) and (
...
class ConfigDiagramCallback(Callback):
    ...
    def __monitor_git():
        try:
            forbidden = False
            with filelock.FileLock(srccode.main.metadata["common_dir"] + "/diff.lock"):
                repo = git.Repo(
                    path=srccode.main.metadata["initial_working_directory"],
                ...
                if repo.is_dirty(untracked_files=True):
                    ...
                    forbidden = (
                        "disallow_uncommitted" in srccode.main.config
                        and srccode.main.config.disallow_uncommitted
                        and not is_in_debug_mode()
                    )
                    untracked = repo.untracked_files
                    if not os.path.exists(
                        srccode.main.metadata["common_dir"] + "/changes.diff"
                    ):
                        repo.git.add(all=True)
                        with open(
                            srccode.main.metadata["common_dir"] + "/changes.diff", "w"
                        ) as f:
            ...
            repo = None
            if (
                "disallow_uncommitted" in srccode.main.cfg
                and srccode.main.cfg.disallow_uncommitted
                and not is_in_debug_mode()
            ):
                raise Exception(
                    "Running experiments without committing is disallowed. Please, commit your changes."
    ...
    def on_launch(self):
        cfg = srccode.main.config
        metadata = srccode.main.metadata
        ...
        repo, commit_hash = (
            self.__monitor_git()
            if not srccode.main.metadata["no_git"]
            else (None, None)
        )
...
class StateTracker(Callback):
    ...
    def is_target_event(self, obj, method, output):
        return (
            isinstance(obj, srccode.simulator.Simulator)
...
class HistoricalCallback(Callback, ABC):
    ...
    def plot(self, name=None):
        if srccode.main.is_clear_matplotlib_cache_in_callbacks:
            plt.clf()
            plt.cla()
            plt.close()
        if not name:
...
class CalfCallback(HistoricalCallback):
    ...
    def is_target_event(self, obj, method, output):
        return (
            isinstance(obj, srccode.scenario.Scenario)
    ...
    def perform(self, obj, method, output):
        ...
        is_calf = (
            obj.critic.opt_status == srccode.optimizable.optimizers.OptStatus.success
            and obj.policy.opt_status
            == srccode.optimizable.optimizers.OptStatus.success
        )
        if not self.data.empty:
...
class SaveProgressCallback(Callback):
    ...
    def on_launch(self):
        with srccode.main.metadata["report"]() as r:
            r["episode_current"] = 0
    ...
    def on_episode_done(
        ...
        with open(filename, "wb") as f:
            dill.dump(srccode.main.callbacks, f)
        self.log(
...
class ObjectiveSaver(HistoricalCallback):
    ...
    def is_target_event(self, obj, method, output):
        return (
            isinstance(obj, srccode.scenario.RLScenario) and (method == "pre_optimize")
        ) or (
            isinstance(obj, srccode.optimizable.Optimizable)
            and (method == "post_epoch" or method == "post_optimize")
        )

    def perform(self, obj, method, output):
        if isinstance(obj, srccode.scenario.RLScenario) and (method == "pre_optimize"):
        ...
        if isinstance(obj, srccode.optimizable.Optimizable) and (
            method == "post_epoch"
        ):
...
class CriticObjectiveSaver(ObjectiveSaver):
    ...
    def is_target_event(self, obj, method, output):
        return (
            ...
            isinstance(obj, srccode.critic.Critic)
            and (method == "post_epoch" or method == "post_optimize")
        )
...
class PolicyObjectiveSaver(ObjectiveSaver):
    ...
    def is_target_event(self, obj, method, output):
        return (
            ...
            isinstance(obj, srccode.policy.Policy)
            and (method == "post_epoch" or method == "post_optimize")
        )
...
class ValueCallback(HistoricalCallback):
    ...
    def on_episode_done(
        ...
        mlflow.log_metric("episodic_return", -scenario.recent_undiscounted_value)
