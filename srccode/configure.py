...
from srccode.__internal.cues_fix import SrccodeSurvey
...
def configure_user_settings():
    ...
    print(
        "Welcome to [bold magenta]srccode[/bold magenta]! Before we proceed let's quickly configure your setup."
    )
    name = "cores"
    message = "Please select the number of cores that srccode will use by default:"
    scale = range(1, os.cpu_count() + 1)
    fields = [{"name": "cores", "message": "Cores"}]
    cue = SrccodeSurvey(name, message, scale, fields)
    ...
    menu = [
        "No, just keep the artifacts in ./srccode_data/mlflow.",
        "Yes, let me set the URI.",
        'I don\'t even know what "mlflow" is!',
    ]
    msg = "Srccode uses [bold magenta]mlflow[/bold magenta] to store experiment data. By default a subfolder is created in the current working directory to store mlflow artifacts. Would you like to set a custom mlflow URI?"
    ...
    if ans == menu[0]:
        ...
        ok(
            "This 'tracking uri' thing is what lets you specify your own mlflow server, which can be useful if you have a group of people working on the same collection of experiments. Since you probably don't have a server like this yet, we'll just store your mlflow experiment data in [bold magenta]./srccode_data/mlflow[/bold magenta]."
        )
        ok(
            "To view your experiment data via mlflow, all you need to do is:\n [bold grey]cd ./srccode_data[/bold grey]\n [bold grey]mlflow ui[/bold grey]\n Then just click the link that shows up in the logs."
        )
    ok(
        "By the way, don't forget to add [bold magenta]srccode_data/[/bold magenta] to your [bold magenta].gitignore[/bold magenta]! Srccode will always create one of these folders in your current working directory."
    )
    ans = choice(
        "Yes, prevent me from running experiments if I have uncommitted changes.",
        "No, let me run srccode even when my repo has uncommitted changes.",
        message="By default srccode, [bold magenta]will not start unless all changes in your repo are committed[/bold magenta]. This helps to ensure reproducibility. If you're doing serious research, it will help you a lot, but if not, then it might be really annoying. Would you like to keep this feature enabled?",
    )
    user_settings["DISALLOW_UNCOMMITTED"] = "Yes" in ans
    name, email = form(
        "Full name",
        "E-mail",
        message="At the end of each experiment srccode generates a [bold magenta]summary[/bold magenta] that outlines the setup, the outcomes and steps to reproduce the results. When working in a team it is useful to have credentials of the author of the experiment present in this summary. Please provide your [bold magenta]e-mail[/bold magenta] and [bold magenta]e-mail[/bold magenta] address or leave the lines blank.",
    )
    ...
    if os.path.exists(script_dev):
        ans = choice(
            "No, I'm only a user of srccode and I do not intend to modify it.",
            "Yes, I am a developer of srccode.",
        ...
        if "Yes" in ans:
            ...
            try:
                ...
                print(e)
                ok(
                    f"It would seem that something went wrong when setting up your environment for contributing to srccode. Before you can start your project you need to make sure that all the necessary pre-requisites are installed. Please try to determine, what went wrong when running {script_dev}."
                )
                return
    ...
    ok(
        "We're [bold magenta]done[/bold magenta]! Bear in mind that you can always change these settings by running srccode with [bold magenta]--configure[/bold magenta]."
    )
