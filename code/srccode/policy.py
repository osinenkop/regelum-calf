...
from srccode.typing import Weights
...
class PolicyReinforce(PolicyGradient):
    ...
    def objective_function(
        ...
        Note:
            All the arguments are sampled from [`DataBuffer`][srccode.data_buffers.DataBuffer]
            via [`iter_bathes`][srccode.data_buffers.DataBuffer.iter_batches] method
        """
...
class CartPoleEnergyBasedPolicy(Policy):
    ...
    def __init__(
        ...
        """
        super().__init__()
        from srccode.system import CartPole
...
class LunarLanderPIDPolicyNew(Policy):
    ...
    def get_action(self, observation):
        ...
        # ) * self.PID_y_dot.compute_signal(rg.array([observation[0, 4]])).reshape(-1)
        # ) + rg.sign(
        #     rg.cos(observation[0, 2])
...
class LunarLanderPIDPolicyJustLand(Policy):
    """Nominal PID scenario for lunar lander."""
...
class ThreeWheeledRobotDisassembledCLFPolicy(Policy):
    ...
    def __init__(
        ...
        super().__init__(optimizer_config=optimizer_config)
        from srccode.system import ThreeWheeledRobotDynamic
