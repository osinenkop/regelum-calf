...
Note:
    ...
    separate [`Simulator`][srccode.simulator] class, which leverages the update rule to create the corresponding integration scheme.
"""
...
class SystemInterface(srccode.SrccodeBase, ABC):
    ...
    Attributes:
        ...
        _dim_inputs: The number of input variables, typically control inputs. In srccode we commonly use inputs parameter for actions.
    ...
    def _compute_state_dynamics(
        ...
        It is intended to be used internally by the [`SystemInterface.compute_state_dynamics`][srccode.system.SystemInterface.compute_state_dynamics] method
    ...
    def get_observation(
        ...
        [`_get_observation`][srccode.system.SystemInterface._get_observation] and applies additional
...
class Integrator(System):
    """System yielding non-holonomic double integrator when composed with [kinematic three-wheeled robot][srccode.system.ThreeWheeledRobotKinematic].
...
class TwoTank(System):
    ...
    Notes:
        ...
        [`TwoTankReferenced`][srccode.system.TwoTankReferenced].
        It is this [`TwoTankReferenced`](srccode.system.TwoTankReferenced) model
...
class LunarLander(System):
    ...
    Notes:
        ...
        For an alternative version where the observation is differentiated from the state by a
        reference value (specifically, $(0.4, 0.4)$ subtracted from the state), please consult
        [`LunarLanderReferenced`][srccode.system.LunarLanderReferenced].
        It is this [`LunarLanderReferenced`](srccode.system.LunarLanderReferenced) model
...
class LunarLanderReferenced(SystemWithConstantReference):
    ...
    This class represents a tailored version of the [`LunarLander`](srccode.system.LunarLander)
    with specific enhancements to match the instructional framework provided by the [tutorial](../systems/lunar_lander.md).

    Notes:
        The [`LunarLanderReferenced`][srccode.system.LunarLanderReferenced] system varies from the original
        [`LunarLander`][srccode.system.LunarLander] in one significant aspect: the computation of the observation.
        In [`LunarLander`][srccode.system.LunarLander], the observation directly equals the current state.
        However, in [`LunarLanderReferenced`][srccode.system.LunarLanderReferenced],
        ...
        and are consistent with the [`LunarLander`][srccode.system.LunarLander] class.
...
class TwoTankReferenced(SystemWithConstantReference):
    ...
    This class represents a tailored version of the [`TwoTank`][srccode.system.TwoTank]
    with specific enhancements to match the instructional framework provided by the [tutorial](../systems/2tank.md).

    Notes:
        The [`TwoTankReferenced`][srccode.system.TwoTankReferenced] system varies from the original
        [`TwoTank`][srccode.system.TwoTank] in one significant aspect: the computation of the observation.
        In [`TwoTank`][srccode.system.TwoTank], the observation directly equals the current state.
        However, in [`TwoTankReferenced`][srccode.system.TwoTankReferenced],
        ...
        and are consistent with the [`TwoTank`][srccode.system.TwoTank] class.
        This ensures that while the observation method has been modified, the fundamental
        behavior of the system **state** dynamics stays true to the original model.
