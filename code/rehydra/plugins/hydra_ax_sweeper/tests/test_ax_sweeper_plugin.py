...
from rehydra_plugins.rehydra_ax_sweeper.ax_sweeper import AxSweeper
...
def test_chunk_method_for_valid_inputs(n: int, expected: List[List[int]]) -> None:
    from rehydra_plugins.rehydra_ax_sweeper._core import CoreAxSweeper
...
def test_jobs_dirs(rehydra_sweep_runner: TSweepRunner) -> None:
    ...
    sweep = rehydra_sweep_runner(
        ...
        overrides=[
            ...
            "rehydra.sweeper.ax_config.max_trials=6",
            "rehydra.sweeper.max_batch_size=2",
            "params=basic",
        ],
    )
    with sweep:
...
def test_jobs_configured_via_config(
    rehydra_sweep_runner: TSweepRunner, test_conf: str
) -> None:
    sweep = rehydra_sweep_runner(
        calling_file="tests/test_ax_sweeper_plugin.py",
        calling_module=None,
        ...
        config_path="config",
        config_name="config.yaml",
        overrides=["rehydra/launcher=basic", f"params={test_conf}"],
    )
    with sweep:
...
def test_jobs_configured_via_cmd(
    rehydra_sweep_runner: TSweepRunner, test_conf: str, override: str, expected_x: float
) -> None:
    sweep = rehydra_sweep_runner(
        calling_file="tests/test_ax_sweeper_plugin.py",
        calling_module=None,
...
def test_jobs_configured_via_cmd_and_config(rehydra_sweep_runner: TSweepRunner) -> None:
    sweep = rehydra_sweep_runner(
        calling_file="tests/test_ax_sweeper_plugin.py",
        calling_module=None,
        task_function=quadratic,
        ...
        overrides=[
            "rehydra/launcher=basic",
            "rehydra.sweeper.ax_config.max_trials=2",
...
def test_configuration_set_via_cmd_and_default_config(
    ...
    sweep = rehydra_sweep_runner(
        ...
        overrides=[
            "rehydra/launcher=basic",
            "rehydra.sweeper.ax_config.max_trials=2",
            "rehydra.sweeper.ax_config.early_stop.max_epochs_without_improvement=2",
            "quadratic=basic",
...
def test_jobs_using_choice_between_lists(
    ...
    cmd = [
        ...
        "rehydra.run.dir=" + str(tmpdir),
        "rehydra.job.chdir=True",
        "rehydra.sweeper.ax_config.max_trials=3",
    ] + [cmd_arg]
    result, _ = run_python_script(cmd)
