...
from rehydra_plugins.rehydra_nevergrad_sweeper import _impl
from rehydra_plugins.rehydra_nevergrad_sweeper.nevergrad_sweeper import NevergradSweeper
...
def test_launched_jobs(rehydra_sweep_runner: TSweepRunner) -> None:
    budget = 8
    sweep = rehydra_sweep_runner(
        ...
        task_function=None,
        overrides=[
            "rehydra/sweeper=nevergrad",
            "rehydra/launcher=basic",
            f"rehydra.sweeper.optim.budget={budget}",  # small budget to test fast
            "rehydra.sweeper.optim.num_workers=3",
            "foo=1,2",
            "bar=4:8",
        ],
    )
...
def test_nevergrad_example(with_commandline: bool, tmpdir: Path) -> None:
    ...
    cmd = [
        ...
        f"rehydra.sweeper.optim.budget={budget}",  # small budget to test fast
        f"rehydra.sweeper.optim.num_workers={min(8, budget)}",
        "rehydra.sweeper.optim.seed=12",  # avoid random failures
    ]
    if with_commandline:
        cmd += [
            "db=mnist,cifar",
...
def test_failure_rate(max_failure_rate: float, tmpdir: Path) -> None:
    cmd = [
        ...
        "rehydra.sweeper.optim.budget=2",  # small budget to test fast
        "rehydra.sweeper.optim.num_workers=2",
        f"rehydra.sweeper.optim.max_failure_rate={max_failure_rate}",
        "error=true",
    ]
