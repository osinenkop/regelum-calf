...
from rehydra_plugins.rehydra_optuna_sweeper import _impl
from rehydra_plugins.rehydra_optuna_sweeper._impl import OptunaSweeperImpl
from rehydra_plugins.rehydra_optuna_sweeper.config import Direction
from rehydra_plugins.rehydra_optuna_sweeper.optuna_sweeper import OptunaSweeper
...
def test_launch_jobs(rehydra_sweep_runner: TSweepRunner) -> None:
    sweep = rehydra_sweep_runner(
        ...
        task_function=None,
        overrides=[
            "rehydra/sweeper=optuna",
            "rehydra/launcher=basic",
            "rehydra.sweeper.n_trials=8",
            "rehydra.sweeper.n_jobs=3",
        ],
    )
    with sweep:
...
def test_optuna_example(with_commandline: bool, tmpdir: Path) -> None:
    ...
    cmd = [
        "example/sphere.py",
        "--multirun",
        "rehydra.sweep.dir=" + str(tmpdir),
        "rehydra.job.chdir=True",
        "rehydra.sweeper.n_trials=20",
        "rehydra.sweeper.n_jobs=1",
        f"rehydra.sweeper.storage={storage}",
        f"rehydra.sweeper.study_name={study_name}",
        "rehydra/sweeper/sampler=tpe",
        "rehydra.sweeper.sampler.seed=123",
        "~z",
    ]
    if with_commandline:
        cmd += [
    ...
    # See https://github.com/facebookresearch/rehydra/pull/1746#discussion_r681549830.
...
def test_example_with_grid_sampler(
    ...
    cmd = [
        ...
        "rehydra.sweep.dir=" + str(tmpdir),
        "rehydra.job.chdir=False",
        f"rehydra.sweeper.n_trials={num_trials}",
        "rehydra.sweeper.n_jobs=1",
        f"rehydra.sweeper.storage={storage}",
        f"rehydra.sweeper.study_name={study_name}",
    ]
...
def test_optuna_multi_objective_example(with_commandline: bool, tmpdir: Path) -> None:
    cmd = [
        ...
        "rehydra.sweeper.n_trials=20",
        "rehydra.sweeper.n_jobs=1",
        "rehydra/sweeper/sampler=random",
        "rehydra.sweeper.sampler.seed=123",
    ]
    if with_commandline:
        cmd += [
...
@mark.parametrize(
    ...
    [
        ...
        (
            ...
            {},
            True,
            r"Both rehydra.sweeper.params and rehydra.sweeper.search_space are configured.*",
        ),
        (
            {},
            None,
            True,
            r"`rehydra.sweeper.search_space` is deprecated and will be removed in the next major release.*",
        ),
        (None, {}, False, None),
    ],
)
...
def test_failure_rate(max_failure_rate: float, tmpdir: Path) -> None:
    cmd = [
        ...
        "rehydra.job.chdir=True",
        "rehydra.sweeper.n_trials=20",
        "rehydra.sweeper.n_jobs=2",
        "rehydra/sweeper/sampler=random",
        "rehydra.sweeper.sampler.seed=123",
        f"rehydra.sweeper.max_failure_rate={max_failure_rate}",
        "error=true",
    ]
