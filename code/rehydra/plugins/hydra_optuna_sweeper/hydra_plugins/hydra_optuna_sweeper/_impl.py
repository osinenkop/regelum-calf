...
class OptunaSweeperImpl(Sweeper):
    ...
    def _process_searchspace_config(self) -> None:
        url = "https://rehydra.cc/docs/upgrades/1.1_to_1.2/changes_to_sweeper_config/"
        if self.params is None and self.search_space is None:
            self.params = OmegaConf.create({})
        elif self.search_space is not None:
            if self.params is not None:
                warnings.warn(
                    "Both rehydra.sweeper.params and rehydra.sweeper.search_space are configured."
                    "\nRehydra will use rehydra.sweeper.params for defining search space."
                    f"\n{url}"
                )
            else:
                deprecation_warning(
                    message=dedent(
                        f"""\
                        `rehydra.sweeper.search_space` is deprecated and will be removed in the next major release.
                        Please configure with `rehydra.sweeper.params`.
                        {url}
                        """
                    ),
                )
    ...
    def setup(
        ...
        *,
        ...
        self.launcher = Plugins.instance().instantiate_launcher(
            config=config, rehydra_context=rehydra_context, task_function=task_function
        )
        self.sweep_dir = config.rehydra.sweep.dir
    ...
    def sweep(self, arguments: List[str]) -> None:
        ...
        OmegaConf.save(
            ...
            f"{self.config.rehydra.sweep.dir}/optimization_results.yaml",
        )
