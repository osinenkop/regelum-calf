...
class RayLauncherConf:
    _target_: str = "rehydra_plugins.rehydra_ray_launcher.ray_launcher.RayLauncher"
...
class RayProviderConf:
    ...
    key_pair: Dict[str, str] = field(
        default_factory=lambda: {"key_name": "rehydra-${oc.env:USER,user}"}
    )
...
def _pip_pkgs_default_factory():
    d = {
        ...
        "rehydra_core": "${ray_pkg_version:rehydra}",
        "ray": "${ray_pkg_version:ray}",
        "cloudpickle": "${ray_pkg_version:cloudpickle}",
        "rehydra_ray_launcher": get_distribution("rehydra_ray_launcher").version,
    }
...
class EnvSetupConf:
    ...
    commands: List[str] = field(
        default_factory=lambda: [
            "conda create -n rehydra_${python_version:micro} python=${python_version:micro} -y",
            "echo 'export PATH=\"$HOME/anaconda3/envs/rehydra_${python_version:micro}/bin:$PATH\"' >> ~/.bashrc",
        ]
    )
...
class RayAWSLauncherConf:
    _target_: str = "rehydra_plugins.rehydra_ray_launcher.ray_aws_launcher.RayAWSLauncher"
