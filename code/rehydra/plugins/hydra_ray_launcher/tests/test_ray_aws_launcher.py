...
from rehydra.test_utils.test_utils import chdir_rehydra_root, chdir_plugin_root
from omegaconf import OmegaConf
from pytest import fixture, mark

from rehydra_plugins.rehydra_ray_launcher.ray_aws_launcher import (  # type: ignore
    RayAWSLauncher,
)
...
temp_remote_dir = "/tmp/rehydra_test/"  # nosec
...
pip_lib_skip = [
    ...
    "cloudpickle",
    "pickle5",
    "rehydra_ray_launcher",
]

common_overrides = [
    f"rehydra.launcher.ray.cluster.cluster_name={cluster_name}",
    "rehydra.launcher.stop_cluster=False",
    "rehydra.launcher.logging.log_style='auto'",
    "rehydra.launcher.logging.color_mode='auto'",
    "rehydra.launcher.logging.verbosity=1",
    "rehydra.launcher.create_update_cluster.no_restart=False",
    "rehydra.launcher.create_update_cluster.restart_only=False",
    "rehydra.launcher.create_update_cluster.no_config_cache=False",
    "rehydra.launcher.teardown_cluster.workers_only=False",
    "rehydra.launcher.teardown_cluster.keep_min_workers=False",
    f"rehydra.launcher.ray.cluster.provider.key_pair.key_name=rehydra_test_{cluster_name}",
    ...
    "rehydra.launcher.ray.cluster.setup_commands=[]",
    "rehydra.launcher.env_setup.commands=[]",
    f"+rehydra.launcher.ray.cluster.available_node_types={ray_nodes_conf_override}",
]
common_overrides.extend(
    [f"~rehydra.launcher.env_setup.pip_packages.{lib}" for lib in pip_lib_skip]
)
...
def build_core_wheel(tmp_wheel_dir: str) -> str:
    ...
    # download dependency wheel for rehydra-core
...
def upload_and_install_wheels(
    ...
    log.info(f"Install rehydra-core wheel {core_wheel}")
    sdk.run_on_cluster(
        connect_config,
...
def manage_cluster() -> Generator[None, None, None]:
    ...
    connect_config = {
        ...
        "provider": {
            ...
            "key_pair": {"key_name": f"rehydra_test_{cluster_name}"},
        },
        "auth": {"ssh_user": "ubuntu"},
        "setup_commands": [
            f"echo 'export PATH=\"$HOME/anaconda3/envs/rehydra_{cur_py_version}/bin:$PATH\"' >> ~/.bashrc"
        ],
        "head_setup_commands": [],
...
launcher_test_suites_overrides = [
    f"rehydra.launcher.sync_down.source_dir={sweep_dir}/",
    f"rehydra.launcher.sync_down.target_dir={sweep_dir}",
]
...
integration_tests_override = [
    "-m",
    "rehydra/launcher=ray_aws",
    f"rehydra.launcher.sync_down.source_dir={temp_remote_dir}/",
    f"rehydra.launcher.sync_down.target_dir={temp_remote_dir}",
]
