...
class BashCompletion(CompletionPlugin):
    def install(self) -> None:
        ...
        script = (
            f"export _REHYDRA_OLD_COMP=$(complete -p {self._get_exec()} 2> /dev/null)\n"
        )
        script += """rehydra_bash_completion()
{
    words=($COMP_LINE)
...
COMP_WORDBREAKS=$COMP_WORDBREAKS complete -o nospace -o default -F rehydra_bash_completion """
        print(script + self._get_exec())

    def uninstall(self) -> None:
        print("unset rehydra_bash_completion")
        print(os.environ.get("_REHYDRA_OLD_COMP", ""))
        print("unset _REHYDRA_OLD_COMP")
