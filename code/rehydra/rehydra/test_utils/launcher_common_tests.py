...
from rehydra.test_utils.test_utils import (
    TSweepRunner,
    integration_test,
    verify_dir_outputs,
)
...
@mark.usefixtures("rehydra_restore_singletons")
class LauncherTestSuite:
    ...
    def test_sweep_1_job(
        self,
        rehydra_sweep_runner: TSweepRunner,
        launcher_name: str,
        overrides: List[str],
        tmpdir: Path,
    ) -> None:
        sweep_1_job(
            rehydra_sweep_runner,
            overrides=["rehydra/launcher=" + launcher_name] + overrides,
    ...
    def test_not_sweeping_rehydra_overrides(
        self,
        rehydra_sweep_runner: TSweepRunner,
        launcher_name: str,
        overrides: List[str],
        ...
        with raises(
            RehydraException,
            match=re.escape(
                "Sweeping over Rehydra's configuration is not supported : 'rehydra.verbose=true,false'"
            ),
        ):
            with rehydra_sweep_runner(
                calling_file=None,
                calling_module="rehydra.test_utils.a_module",
                task_function=None,
                config_path="configs",
                config_name="compose.yaml",
                overrides=overrides
                + [
                    "rehydra/launcher=" + launcher_name,
                    "+a=0,1",
                    "rehydra.verbose=true,false",
                ],
                temp_dir=tmpdir,
            ):
                pass
    ...
    def test_sweep_1_job_strict_and_bad_key(
        ...
        with raises(Exception):
            sweep_1_job(
                rehydra_sweep_runner,
                overrides=["rehydra/launcher=" + launcher_name, "boo=bar"] + overrides,
                task_function=self.get_task_function(),
                temp_dir=tmpdir,
            )
    ...
    def test_sweep_over_unspecified_mandatory_default(
        ...
        base_overrides = ["rehydra/launcher=" + launcher_name, "group1=file1,file2"]
        sweep = rehydra_sweep_runner(
            calling_file=None,
            calling_module="rehydra.test_utils.a_module",
            task_function=self.get_task_function(),
            config_path="configs",
    ...
    def test_sweep_with_custom_resolver(
        ...
        overrides1 = ["rehydra/launcher=" + launcher_name] + overrides
...
class BatchedSweeperTestSuite:
    def test_sweep_2_jobs_2_batches(
        ...
        rehydra_overrides = ["rehydra/launcher=" + launcher_name]
        overrides.extend(job_overrides)
        overrides.extend(rehydra_overrides)
        sweep = rehydra_sweep_runner(
            calling_file=None,
            calling_module="rehydra.test_utils.a_module",
            task_function=None,
        ...
        with sweep:
            ...
            assert multirun_cfg.rehydra.overrides.task == job_overrides
...
def sweep_1_job(
    ...
    with sweep:
        ...
        assert job_ret[0].rehydra_cfg.rehydra.job.name == "a_module", (
            "Unexpected job name: " + job_ret[0].rehydra_cfg.rehydra.job.name
        )
        assert job_ret[0].rehydra_cfg.rehydra.job.id is not None
        assert job_ret[0].rehydra_cfg.rehydra.job.num is not None
...
def sweep_2_jobs(
    ...
    with sweep:
        ...
        for i in range(2):
            ...
            assert job_ret.rehydra_cfg.rehydra.job.name == "a_module", (
                "Unexpected job name: " + job_ret.rehydra_cfg.rehydra.job.name
            )
            assert job_ret.rehydra_cfg.rehydra.job.id is not None
            assert job_ret.rehydra_cfg.rehydra.job.num is not None
...
class IntegrationTestSuite:
    ...
    def generate_custom_cmd(self) -> Callable[..., List[str]]:
        ...
        def fun(cmd: List[str], filename: str) -> List[str]:
            ...
            param filename: file name to be executed as main rehydra module
    ...
    @mark.parametrize(
        ...
        [
            ...
            param(
                None,
                ["rehydra.job.name=overridden_name"],
                "no_config.py",
                "overridden_name",
                id="different_filename",
            ),
            param(
                {"rehydra": {"job": {"name": "name_from_config_file"}}},
                [],
                "with_config.py",
    ...
    def test_custom_task_name(
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints="RehydraConfig.get().job.name",
    ...
    @mark.parametrize(
        ...
        [
            param(
                {
                    "rehydra": {
                        "sweep": {
                            "dir": "task_cfg",
                            "subdir": "task_cfg_${rehydra.job.num}",
                        }
                    }
                },
                [],
            ...
            param(
                {},
                [
                    "rehydra.sweep.dir=cli_dir",
                    "rehydra.sweep.subdir=cli_dir_${rehydra.job.num}",
                ],
                "cli_dir/cli_dir_0",
            ...
            param(
                {
                    "rehydra": {
                        "sweep": {
                            "dir": "rehydra_cfg",
                            "subdir": "${rehydra.job.override_dirname}",
                        }
                    },
                    "a": "hello",
                    "b": 20,
    ...
    def test_custom_sweeper_run_workdir(
        ...
        integration_test(
            tmpdir=self.get_test_scratch_dir(tmpdir),
            task_config=cfg,
            overrides=overrides + ["rehydra.job.chdir=True"],
            prints="os.getcwd()",
    ...
    def test_get_orig_dir_multirun(
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints="rehydra.utils.get_original_cwd()",
    ...
    def test_to_absolute_path_multirun(
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints="rehydra.utils.to_absolute_path('/foo/bar')",
        ...
        integration_test(
            ...
            task_config=cfg,
            overrides=overrides,
            prints=["rehydra.utils.to_absolute_path('foo/bar')", "os.getcwd()"],
